...
export function ChurnAnalysisChart({
  ...
  const getChurnStatus = (rate: number) => {
    if (rate <= 2) return { label: "Excellent", color: "text-green-500", bgColor: "bg-green-500" }
    if (rate <= 5) return { label: "Good", color: "text-blue-500", bgColor: "bg-blue-500" }
    if (rate <= 8) return { label: "Moderate", color: "text-yellow-500", bgColor: "bg-yellow-500" }
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Loading churn metrics...</CardDescription>
        </CardHeader>
        <CardContent>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle>Churn Analysis</CardTitle>
            <CardDescription>Customer and revenue churn metrics</CardDescription>
          </div>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="p-4 rounded-lg border">
            <div className="flex items-center justify-between">
              <span className="text-sm text-muted-foreground">Customer Churn</span>
            ...
            <div className="text-xs text-muted-foreground">
              {churnStatus.label} - Last period
            </div>
          </div>

          <div className="p-4 rounded-lg border">
            <div className="text-sm text-muted-foreground">Revenue Churn (MRR)</div>
          ...
          <div className="p-4 rounded-lg border">
            ...
            <div className="text-xs text-muted-foreground">
              {netRevenueRetention >= 100 ? "Growing from existing" : "Net contraction"}
            </div>
          </div>
        </div>
        ...
        {data.length > 0 && (
          <div>
            <h4 className="text-sm font-medium mb-3">Churn Trend</h4>
                  ...
                  >
                    ...
                    <div className="absolute bottom-full mb-2 hidden group-hover:block z-10">
                      <div className="bg-popover text-popover-foreground rounded-md shadow-lg p-2 text-xs whitespace-nowrap">
                        ...
                        <div>Customer: {point.customerChurnRate.toFixed(2)}%</div>
                        <div>Revenue: {point.revenueChurnRate.toFixed(2)}%</div>
                        <div>Lost: {point.churnedCustomers} customers</div>
                        <div>Lost: {formatCurrency(point.churnMrr)}</div>
                      </div>
                    </div>
        ...
        <div className="pt-4 border-t">
          <h4 className="text-sm font-medium mb-3">Churn Benchmarks</h4>
          <div className="space-y-3">
            <div>
              <div className="flex items-center justify-between text-sm mb-1">
                <span>Your Customer Churn</span>
              ...
              <div className="relative">
                ...
                {/* Benchmark markers */}
                <div className="absolute top-0 left-[20%] h-2 w-0.5 bg-green-500" title="Excellent: 2%" />
                <div className="absolute top-0 left-[50%] h-2 w-0.5 bg-yellow-500" title="Average: 5%" />
                <div className="absolute top-0 left-[80%] h-2 w-0.5 bg-red-500" title="High: 8%" />
              </div>
              <div className="flex justify-between text-xs text-muted-foreground mt-1">
                <span>0%</span>
                <span className="text-green-500">2% Excellent</span>
                <span className="text-yellow-500">5% Average</span>
                <span className="text-red-500">8%+ High</span>
              </div>
            </div>
          </div>
        </div>
