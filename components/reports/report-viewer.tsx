          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {t("generatedBy", { agent: reportData.agent, date: new Date(reportData.updatedAt).toLocaleDateString() })}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Sheet>
              ...
              <SheetContent>
                <SheetHeader>
                  <SheetTitle>{t("enhance.title")}</SheetTitle>
                  <SheetDescription>{t("enhance.description")}</SheetDescription>
                </SheetHeader>
                <div className="mt-6 space-y-4">
                  <Button variant="outline" className="w-full justify-start bg-transparent">
                    <RefreshCw className="mr-2 h-4 w-4" />
                    {t("enhance.refreshData")}
                  </Button>
                  <Button variant="outline" className="w-full justify-start bg-transparent">
                    <PenLine className="mr-2 h-4 w-4" />
                    {t("enhance.improveWriting")}
                  </Button>
                  <Button variant="outline" className="w-full justify-start bg-transparent">
                    <Layers className="mr-2 h-4 w-4" />
                    {t("enhance.addVisualizations")}
                  </Button>
                  <Button variant="outline" className="w-full justify-start bg-transparent">
                    <MessageSquare className="mr-2 h-4 w-4" />
                    {t("enhance.generateSummary")}
                  </Button>
                </div>
              </SheetContent>
            </Sheet>
              ...
              <DialogContent>
                ...
                <div className="space-y-4 py-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium">{t("share.inviteByEmail")}</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium">{t("share.shareLink")}</label>
                    <div className="flex gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem>{t("menu.schedule")}</DropdownMenuItem>
                <DropdownMenuItem>{t("menu.viewApi")}</DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem className="text-destructive">{t("menu.delete")}</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </div>
          ...
          <Tabs defaultValue="details" className="flex-1 flex flex-col">
            <TabsList className="w-full justify-start rounded-none border-b h-auto p-0 shrink-0">
              ...
              <TabsTrigger
                ...
                className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary px-4 py-3"
              >
                {t("tabs.comments", { count: reportData.comments.length })}
              </TabsTrigger>
              <TabsTrigger
                value="citations"
                className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary px-4 py-3"
            ...
            <TabsContent value="details" className="flex-1 p-4 space-y-6 m-0 overflow-auto">
              <div>
                <h3 className="font-medium mb-2">{t("details.description")}</h3>
                <p className="text-sm text-muted-foreground">{reportData.description}</p>
              </div>

              <div>
                <h3 className="font-medium mb-2">{t("details.author")}</h3>
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2 text-muted-foreground mb-1">
                      ...
                      <span className="text-xs">{t("details.views")}</span>
                    </div>
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2 text-muted-foreground mb-1">
                      ...
                      <span className="text-xs">{t("details.slides")}</span>
                    </div>
              ...
              <div>
                <h3 className="font-medium mb-2">{t("details.recentActivity")}</h3>
            ...
            <TabsContent value="comments" className="flex-1 flex flex-col m-0 overflow-hidden">
                  ...
                  <div key={comment.id} className="space-y-2">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        >
                          {t("comments.viewSlide", { slide: comment.slide })}
                        </button>
                      </div>
                    </div>
                  </div>
              ...
              <div className="p-4 border-t shrink-0">
                ...
                <div className="flex justify-between items-center mt-2">
                  <span className="text-xs text-muted-foreground">{t("comments.commentingOnSlide", { slide: currentSlide + 1 })}</span>
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {t("sources.dataPointsMarkets", { dataPoints: citation.dataPoints, markets: citation.markets })}
                          </p>
                        </div>
                      </div>
                      <Badge variant="outline" className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20">
            ...
            <TabsContent value="versions" className="flex-1 p-4 space-y-4 m-0 overflow-auto">
              <p className="text-sm text-muted-foreground">{t("versions.description")}</p>
